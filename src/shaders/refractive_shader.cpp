...
vec3 Refractive_Shader::Shade_Surface(const Ray &ray,
    ...
    if (kr < 1) {
        ...
        vec3 refractionRayOrig = outside ? intersection_point - normal : intersection_point + normal;
        Hit closest = world.Closest_Intersection(Ray(refractionRayOrig, refractionDirection));
        if (closest.object != nullptr) {
            vec3 point = intersection_point + closest.dist * refractionDirection;
            color += (1 - color_intensity) * (1 - kr) * closest.object->material_shader->Shade_Surface(
                                        Ray(intersection_point, refractionDirection), point,
                                        closest.object->Normal(point, closest.part),
                                        recursion_depth + 1);
        } else {
            color += (1 - color_intensity) * (1 - kr) * world.background_shader->Shade_Surface(
    ...
    vec3 reflectionRayOrig = outside ? intersection_point + normal : intersection_point - normal;
    Hit closest = world.Closest_Intersection(Ray(reflectionRayOrig, reflectionDirection));
    if (closest.object != nullptr) {
        vec3 point = intersection_point + closest.dist * reflectionDirection;
        color += (1 - color_intensity) * kr * closest.object->material_shader->Shade_Surface(
                                    Ray(intersection_point, reflectionDirection), point,
                                    closest.object->Normal(point, closest.part),
                                    recursion_depth + 1);
    } else {
        color += (1 - color_intensity) * kr * world.background_shader->Shade_Surface(
                                    Ray(intersection_point, reflectionDirection),
                                    {0, 0, 0}, {0, 0, 0}, 0);
    }
...
void Refractive_Shader::fresnel(const vec3 &direction, const vec3 &normal, double &kr) const{
...
vec3 Refractive_Shader::refract(const vec3 &direction, const vec3 &normal) const {
...
vec3 Refractive_Shader::reflect(const vec3 &direction, const vec3 &normal) const {
