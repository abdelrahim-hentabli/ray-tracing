...
vec3 Reflective_Shader::Shade_Surface(const Ray &ray,
  ...
  if (closest.object != nullptr) {
    ...
    color += reflectivity * closest.object->material_shader->Shade_Surface(
                                Ray(intersection_point, reflectionDir), point,
                                closest.object->Normal(point, closest.part),
                                recursion_depth + 1);
  } else {
    color += reflectivity * world.background_shader->Shade_Surface(
                                Ray(intersection_point, reflectionDir),
                                {0, 0, 0}, {0, 0, 0}, 0);
  }
  return color;
}
